...
BBV::BBV(int size_v) {
  if (size_v > 0) {
    vec = new byte[(size_v - 1) / 8 +
                   1]; // избегаем случай размера кратного восьми
    if (vec != NULL) // если выделилась память, то заполняем поля размера
                     // массива, кол-ва ячеек
    {
      len = size_v;
      size = (size_v - 1) / 8 + 1;
...
BBV::BBV(const char *str) {
  ...
  {
    len = strlen(str); // заполняем поля размера вектора и вычисляем кол-во
                       // ячеек
    size = (len - 1) / 8 + 1;
    vec = new byte[size];
    if (vec != NULL) {
      ...
      while (i < len) // проходим по всем битам и записываем в них
...
BBV::BBV(BBV &V) {
  if (V.vec != NULL) // проверяем существование вестора V.vec
  {
    ...
    len = V.len;
    vec = new byte[size]; // захватываем память под новый веторк и переписываем
                          // в него значение из переданного вектора
    if (vec != NULL) {
...
void BBV::Init(const char *str) {
  if (str != NULL) // првоериям существует ли строка
  {
    ...
    if (vec != NULL) // если вектор сущестовал, то мы его удаляем
      delete vec;
    vec = new byte[size]; // захватываем память под новый вектор и переписываем
                          // в него значения из строки
    if (vec != NULL) {
      int i = 0, j = 8, k = 0;
      byte mask = 1;
      vec[0] ^= vec[0];
...
void BBV::Set0(int k) {
  if (vec != NULL && k >= 0 &&
      k < len) // проверяем существование вектора и корректность индекса
  {
    byte mask = 1;
...
BBV BBV::operator=(BBV &V) {
  if (this != &V) // проверка на самоприсвоение
  {
    ...
    if (vec != NULL)
      delete vec;
    vec = new byte[size]; // захватываем память под новый вектор и переписывем
                          // туда переданный вектор
    if (vec != NULL) {
      for (int i = 0; i < size; i++)
        vec[i] = V.vec[i];
    } else
...
BBV BBV::operator=(const char *str) {
  if (str != NULL) // проверяем существует ли строка
  {
    ...
    if (vec != NULL)
      delete vec;
    vec = new byte[size]; // захватываем память для нового вектора и
...
bool BBV::operator==(BBV &V) {
  if (vec != NULL && V.vec != NULL &&
      V.len == len) // проверка существования векторов и равенство их длин
  {
...
BBV BBV::operator|(BBV &V) {
  if (vec != NULL && V.vec != NULL &&
      len == V.len) // проверка существования векторов и равенство их длин
  {
...
BBV BBV::operator>>(int k) {
  BBV res(*this); // создаем результирующий вектор
  if (vec != NULL && k > 0) {
    for (int i = 0; i < size; i++) // обнуляем все ячейки результирующего
                                   // вектора
      res.vec[i] ^= res.vec[i];
    ...
    for (int i = 0; start + i < size; i++) {
      ...
      res.vec[start + i] |=
          mask >>
          (8 -
           k); // дописали перенесенную часть, которая вышла за пределы ячейки
      mask = -1;
      mask <<= 8 - k;
...
BBV BBV::operator<<(int k) {
  ...
  if (vec != NULL && k > 0) {
    for (int i = 0; i < size; i++) // обнуляем все ячейки результирующего
      ...
      res.vec[i] ^= res.vec[i];
    int start =
        size - 1 - (k - 1) / 8; // первая ячейка для записи в векторе res
    if (k % 8 == 0)
      start = size - 1 - k / 8;
    k %= 8; // сдвиг в каждой ячейке
    byte mask = 0;
    for (int i = 0; start - i >= 0; i++) {
      res.vec[start - i] =
          vec[size - 1 - i] >>
          k; // записываем сдвинутую ячейку, которая вышла за пределы ячейки
...
X BBV::operator[](int k) {
  if (vec != NULL && k >= 0 && k < len) {
    X ptr(&(vec[k / 8]), k % 8); // формируем объект типа X, куда помещаем
                                 // указатель нужной ячейки и номер бита
    return ptr;
  } else
    throw 1;
}
...
BBV::operator char *() {
  if (vec != NULL) // проверяем существование вектора
  {
    ...
    if (str != NULL) {
      ...
      byte mask = 1;
      for (int i = 0; i < len;
           i++) // переписываем значения из битов в ячейки строки
      {
        if (i % 8 == 0 && i != 0) {
          k++;
          mask = 1;
...
X::X(byte *vec, int k) // передаем в конструктор указатель на нужную ячейку и
                       // номер бита в ней
{
  if (vec != NULL && k >= 0 &&
      k <= 7) // проверяем допустимость номера бита и существование ячейки
  {
    ptr = vec;
    index = k;
  } else
...
X X::operator=(int k) {
  if (ptr != NULL) // проверяем существует ли ячейка с битом, в который будет
                   // производится запись значения
  {
    byte mask =
        1; // подготавливаем маску с правильно размещенным битом со значение 1
    mask <<= index;
...
X::operator int() {
  if (ptr != NULL) // проверяем сущестование ячейки с нужным битом
  {
    ...
    mask <<= index;
    if (*ptr & mask) // если предположение раввенства 0 не верно, запишем в k
                     // новое значение 1
      k = 1;
    return k;
  } else
...
ostream &operator<<(ostream &r, BBV &V) {
  if (V.vec != NULL) // првоверяем существование ячеек булева вектора
  {
    ...
    {
      if (j > 0) // в каждую ячейку заходим 8 раз. счетчик j отвечает за
                 // количество заходов в ячейку
      {
        if (V.vec[k] &
            mask) // с помощью маски получаем значение бита и выводим его
          cout << '1';
        else
          cout << '0';
        mask <<= 1;
...
istream &operator>>(istream &r, BBV &V) // return r;
{
  ...
  char *str =
      new char[str_size + 1]; // создаем строку для вектора у учетом символа \0
  if (str != NULL) // если память выделилась, то вводим в строку и
                   // инициализируем ей вектор
  {
    cout << "Input BBV:\n";
    cin >> str;
    V.Init(str);
