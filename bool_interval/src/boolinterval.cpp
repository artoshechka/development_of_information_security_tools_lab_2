...
BoolInterval::BoolInterval(const char *vec_in, const char *dnc_in) {
  // Проверяем , если строки не пустые и их длины совпадают, то инициализируем
  // вектора, иначе ошибка и в этом случае создаем нулевые веткора мин длины(1
  // байт).
...
int BoolInterval::length() { return vec.getSize(); }
