...
Программа поддерживает комбинируемые флаги для разных режимов работы:

#### Базовые команды

| Команда | Описание |
|---------|----------|
| `./program <input_file>` | Решить булево уравнение из файла (стратегия: min-dont-care) |
| `./program <input_file> <strategy>` | Решить с указанной стратегией (min-dont-care, first-free, min, first) |
| `./program --bench` | Запустить бенчмарки аллокатора |
| `./program --test` | Запустить тесты управления памятью в многопоточном режиме |

#### Комбинированные флаги

Флаги `--bench` и `--test` можно использовать вместе:

```bash
# Бенчмарки и тесты
./program --bench --test

# Бенчмарки, тесты и решение уравнения
./program --bench --test ../SatExamples/sat_ex_1.pla

# Бенчмарки, тесты и решение с конкретной стратегией
./program --bench --test ../SatExamples/sat_ex_1.pla first-free

# Только тесты и решение
./program --test ../SatExamples/sat_ex_1.pla
```
...
# Запустить тесты памяти (многопоточность, статистика каждые 30с)
./program --test

# Все вместе: бенчмарки, тесты и решение
./program --bench --test ../SatExamples/sat_ex_1.pla min-dont-care
```
...
**Тесты памяти (`--test`)**
- Запускает 5 тестов управления памятью:
  1. Создание объектов `BoolInterval` 
  2. Создание объектов `BBV`
  3. Обработка ошибок при выделении огромных объемов
  4. Создание объектов `NodeBoolTree`
  5. Многопоточное выделение `BBV` до исчерпания памяти
- В тесте 5 используются все ядра процессора (`std::thread::hardware_concurrency()`)
- Каждые 30 секунд выводится статистика: прошедшее время, число созданных объектов, скорость выделения
- Корректно обрабатывает `std::bad_alloc` и синхронизирует потоки

### Варианты запуска с тестами

```bash
# Только решение
./development_of_information_security_tools_lab_2 ../SatExamples/sat_ex_1.pla

# Бенчмарки аллокатора
./development_of_information_security_tools_lab_2 --bench

# Тесты памяти
./development_of_information_security_tools_lab_2 --test

# Бенчмарки + Решение
./development_of_information_security_tools_lab_2 --bench ../SatExamples/sat_ex_2.pla

# Все вместе
./development_of_information_security_tools_lab_2 --bench --test ../SatExamples/sat_ex_3.pla min
```
