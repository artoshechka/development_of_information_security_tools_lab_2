...
- **BranchingStrategyFactory** — кэширует синглтоны стратегий и возвращает нужную по имени; позволяет менять правило ветвления без изменения логики решателя.
...
Перед первой сборкой необходимо склонировать внешний репозиторий Allocator в корень этого проекта:

```bash
git clone https://github.com/endurodave/Allocator.git
```

Если репозиторий клонирован в другое место, укажите путь при конфигурации:

```bash
cmake -S . -B build -DALLOCATOR_DIR=/полный/путь/к/Allocator
```
...
При запуске программы с флагом `--bench` вызывается `RunAllocatorBenchmarks()`: сначала на одинаковой нагрузке сравниваются четыре варианта — штатный `new[]`/`delete[]`, `HEAP_BLOCKS`, `HEAP_POOL` и `STATIC_POOL` — а затем запускается отдельный exhaustion-сценарий, который удерживает память до возникновения `bad_alloc`. Для каждого режима выводится время работы и число успешных выделений до исчерпания памяти.
